...
adminChangePasswordRouter.post("/", async(req,res) => {
    try {
        ...
        const token = await adminChangePasswordHandler(user_name,oldPassword,newPassword);
...
const adminChangePasswordHandler = async(user_name,oldPassword,newPassword) => {
    try {
        const result = await poolQuery(`select password from admin where username = '${user_name}'`);
