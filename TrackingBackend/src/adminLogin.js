...
adminLoginRouter.post("/", async(req,res) => {
    try {
        ...
        res.json({error: 0, message: "Admin Login Successful", accessToken: token});
    } catch (e) {
...
const adminLogInHandler = async(admin_name, password) => {
    try {
        const result = await poolQuery(`select * from admin where username = '${admin_name}'`);
        ...
        const token = jwtCreator(userId,userName, result.rows[0].role);
